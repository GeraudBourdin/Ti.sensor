...
#define TAG "com.geraudbourdin.sensor"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Sensor_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Sensor.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::SensorBindings::lookupGeneratedInit(
...
static void Sensor_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Sensor_getBinding)->GetFunction());
}

static void Sensor_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::SensorBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry SensorBinding = {
	"com.geraudbourdin.sensor",
	Sensor_init,
	Sensor_dispose
};
...
Java_com_geraudbourdin_sensor_SensorBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.geraudbourdin.sensor", &SensorBinding);
	titanium::KrollBindings::addExternalLookup(&(::SensorBindings::lookupGeneratedInit));
}
